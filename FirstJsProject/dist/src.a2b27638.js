...
function handleCardClick(event) {
  var targetBtn = event.target.closest('.product__add');
  if (!targetBtn) return;
  var card = targetBtn.closest('.product');
  var id = Number(card.dataset.productId);
  var basket = getBasketLocalStorage();
...
function checkingActiveButtons(basket) {
  var buttons = document.querySelectorAll('.product__add');
  buttons.forEach(function (btn) {
    var card = btn.closest('.product');
    var id = Number(card.dataset.productId);
    var isInBasket = basket.includes(id);
...
function updateBasketCount(basket) {
  var basketCount = document.querySelector('.basket__count');
  if (basketCount) {
...
/* time */
var startBtn = document.querySelector('.date');
function startTimer() {
  setInterval(function () {
    ...
    startBtn.innerHTML = "   <div class=\"product\">\n        <p class=\"day\"> ".concat(dayName, "</p>\n        <p class=\"num\">").concat(date, "</p>\n        <p class=\"month\">").concat(monthName, "</p>\n        <p class=\"time\"> ").concat(hours, ":").concat(minutes.toString().padStart(2, '0'), ":").concat(seconds.toString().padStart(2, '0'), "</p>\n      </div> ");
  }, 1000);
}
startTimer();
